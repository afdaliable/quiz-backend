...
pub struct User {
    ...
    pub level_updated_at: Option<DateTime<Utc>>,
...
impl<'c> FromRow<'c, MySqlRow> for User {
    fn from_row(row: &'c MySqlRow) -> Result<Self, sqlx::Error> {
        Ok(User {
            ...
            total_xp: row.try_get("total_xp").unwrap_or(0),
            current_level: row.try_get("current_level").unwrap_or(1),
            level_updated_at: row.try_get("level_updated_at").unwrap_or(None),
